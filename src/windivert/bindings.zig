...
pub fn receive(handle: Handle, buffer: []u8) ReceiveError!ReceiveResult {
    ...
    if (WinDivertRecv(handle, buffer.ptr, @intCast(buffer.len), &result.len, &result.address) == 0) switch (windows.kernel32.GetLastError()) {
...
pub fn send(handle: Handle, buffer: []u8, address: Address) SendError!c_uint {
    ...
    if (WinDivertSend(handle, buffer.ptr, @intCast(buffer.len), &bytes_sent, &address) == 0) return windows.unexpectedError(windows.kernel32.GetLastError());
    return bytes_sent;
}
...
pub fn parsePacket(packet: []const u8) ParsePacketError!ParsedPacket {
    ...
    return .{
        ...
        .payload = payload_ptr[0..@intCast(payload_len)],
    };
}
